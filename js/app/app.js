(function () {
  ...
  app.controller('playerController', function($rootScope, $scope, $http, $stateParams) {
    ...
    $rootScope.search = $rootScope.name;
     ...
     $rootScope.updateQuery = function () {
        /* Retrieve all battleground deserters data */
        $http.get( app.api + "characters/battleground_deserters/" + $rootScope.rows + "?from=" + $rootScope.from + "&name=" + $rootScope.name)
    ...
    $rootScope.updateSearch = (search) => {
      $rootScope.search = search;
      $rootScope.name = search;
    };

    $rootScope.updateQuery();
